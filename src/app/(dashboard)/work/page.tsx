...
import { WorkAssigneeNav } from "@/components/work/assignee-nav";
...
export default async function WorkPage(props: {
  ...
  const [people, categoryRows, total, items] = await Promise.all([
    db.person.findMany({ orderBy: { name: "asc" } }),
    // Every assignee (not just "기타") can have categorized items -- the sidebar drill-down
  ...
  const categoriesByAssignee: Record<string, string[]> = {};
  for (const row of categoryRows) {
    if (!row.category) continue;
    (categoriesByAssignee[row.assigneeName] ??= []).push(row.category);
  }

  const assigneeEntries = [
    ...people.map((person) => ({ name: person.name, isEtc: false })),
    ...(categoriesByAssignee[ETC_ASSIGNEE] ? [{ name: ETC_ASSIGNEE, isEtc: true }] : []),
  ];
    ...
    <main className="mx-auto w-full max-w-6xl space-y-6 px-6 py-10 sm:px-8 sm:py-12">
      ...
      <div className="flex flex-col gap-6 sm:flex-row">
        <WorkAssigneeNav
          ...
          pageSize={pageSize}
          addItemOptions={[...people.map((person) => person.name), ETC_ASSIGNEE]}
        />
